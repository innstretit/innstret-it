...
export const WhyChooseUs: React.FC<WhyChooseUsProps> = ({ onOpenQuoteModal }) => {
  ...
  return (
    ...
    >
      {/* Premium Brand Background Graphic with slow subtle ambient scale (almost subconscious, 1–1.5% max) */}
      <div 
        className="absolute inset-0 bg-cover bg-center bg-no-repeat pointer-events-none opacity-90 transition-transform duration-[7000ms] ease-out will-change-transform scale-[1.015]"
        style={{
          backgroundImage: `url('https://res.cloudinary.com/agu65138/image/upload/v1789323315/backinstreet.png')`,
        }}
      />

      {/* Calibrated Gradient Veil: Keeps typography crisp and high-contrast */}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-left">
        ...
        {/* Eyebrow Label */}
        <div
          className={`mb-6 sm:mb-8 transition-all duration-400 ease-out ${
        ...
        {/* Hero Editorial Manifesto Headline: Sequenced Declaration */}
        <h2 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight text-white leading-[1.08] mb-8 sm:mb-10">
          <span
            className={`inline-block transition-all duration-500 ease-out delay-75 ${
          ...
          <span
            className={`text-[#207BF8] block sm:inline transition-all duration-500 ease-out delay-150 ${
              isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'
            }`}
          >
            Luego la tecnología.
        ...
        <p
          className={`text-lg sm:text-xl md:text-2xl text-gray-300 font-normal leading-relaxed max-w-2xl mb-20 md:mb-28 transition-all duration-500 ease-out delay-200 ${
            isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'
          }`}
        >
        ...
        <div className="relative mb-20 md:mb-28">
          ...
          {/* Extremely fine horizontal connector line in desktop smoothly drawing */}
          <div 
            className={`hidden md:block absolute top-[6px] left-0 right-0 h-[1px] bg-gradient-to-r from-[#207BF8]/80 via-white/20 to-white/10 transition-all duration-700 ease-out delay-250 ${
              isInView ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0 origin-left'
            }`} 
            aria-hidden="true"
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12 relative z-10">
            {steps.map((step, idx) => {
              const delayClasses = ['delay-300', 'delay-400', 'delay-500'][idx] || 'delay-300';

              return (
                <div
                  key={idx}
                  className={`relative group text-left transition-all duration-500 ease-out ${delayClasses} ${
        ...
        <div
          className={`pt-4 transition-all duration-500 ease-out delay-500 ${
            isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'
          }`}
        >
          <button
